...
for i in range(N):
    # 1~3개의 수로 랜덤 지정한다.
    p.append(random.choice(range(2, 5)))
    q.append(random.choice(range(1, 4)))
...
for i in V:
    ...
    for j in V:
        ...
        e[i].append(dist(coordinates[i], coordinates[j]))
...
model.objective = minimize(xsum(g2[k][0] for k in Kc))
...
for (i, j) in product(V, V):
    if i != j:
        for k in Kc:
            model += u1[k][i] - c[i][j]*(Wc+Wp*s[k][i]+Wq*r[k][i])/Wc + M*(1 - x[k][i][j]) >= u2[k][j]
        for k in Ke:
            model += u1[k][i] - e[i][j]*(We+Wp*s[k][i]+Wq*r[k][i])/We + M*(1 - x[k][i][j]) >= u2[k][j]

# GHG배출량 계산
for (i, j) in product(V, V):
    if i != j:
        for k in Kc:
            model += g1[k][i] + rc*c[i][j]*(Wc+Wp*s[k][i]+Wq*r[k][i])/Wc - M*(1 - x[k][i][j]) <= g2[k][j]
...
for (i, j) in product(V, V - {0}):
    if i != j:
        if j in Vn:
            for k in Kc:
                model += s[k][i] - p[j-1]*x[k][i][j] + Cc*(1 - x[k][i][j]) >= s[k][j]
                model += r[k][i] + q[j-1]*x[k][i][j] - Cc*(1 - x[k][i][j]) <= r[k][j]
                
            for k in Ke:
                model += s[k][i] - p[j-1]*x[k][i][j] + Ce*(1 - x[k][i][j]) >= s[k][j]
                model += r[k][i] + q[j-1]*x[k][i][j] - Ce*(1 - x[k][i][j]) <= r[k][j]
        else:
            for k in Kc:
...
if model.num_solutions:
    ...
    for i in V:
        if i == 0:
            ...
            print('%2d\tcustm\t%+.1f\t%+.1f\t%d\t%d'%(i, coordinates[i][0], coordinates[i][1], p[i-1], q[i-1]))
        elif i in Vc:
            print('%2d\tfuels\t%+.1f\t%+.1f'%(i, coordinates[i][0], coordinates[i][1]))
        elif i in Ve:
            print('%2d\tchags\t%+.1f\t%+.1f'%(i, coordinates[i][0], coordinates[i][1]))
    print()
    res_img = makeresimg(SIZE, node_size, route_width, coordinates, V, Vn, Vc, Ve, K, Kc, Ke, Qc, Qe, rc, 1, x, c)
    res_img.show()
